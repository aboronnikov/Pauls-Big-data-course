...
object JsonUtils {
  ...
  /**
    ...
    */
  def transformTweetStringIntoObjects(tweetJson: String): List[TweetInfo] = {

    val dateTime = extractDate(tweetJson).toString
    val hashTags = extractHashtags(tweetJson)
    val userId = extractUserId(tweetJson)

    hashTags
      .map(hashTag => TweetInfo(dateTime, hashTag, userId))
  }
}
