...
class KafkaProducerUtilsTest extends JUnitSuite {
  ...
  def writeToKafkaAsyncTest(): Unit = {
    val bufferedReader = Mockito.mock(classOf[BufferedReader])
    Mockito.when(bufferedReader.readLine()).thenReturn("key,a", "key,b", "key,c", "key,d", "key,e", "key,f", null)
    val producer1 = new MockProducer
    val producer2 = new MockProducer
    val executor = Executors.newFixedThreadPool(2)
    KafkaProducerUtils.writeToKafkaAsync(bufferedReader, producer1, executor, "test")
    KafkaProducerUtils.writeToKafkaAsync(bufferedReader, producer2, executor, "test")
